...
def step(params, opt_state):
    loss_value, grads = jax.value_and_grad(L2_error_log)(
        params
    )  # Gradient of L2 loss w.r.t parameters
...
# Looking at the gradient of the loss is another good way to test convergence. All the gradients are tiny - indicating we have converged.
...
plt.xlabel("$Q$ ($A^{-1}$)")
plt.ylabel("Reflectivity")
plt.legend(["Data", "$\pm 3 \sigma$", "Old model", "Optimised model"])
plt.show()
